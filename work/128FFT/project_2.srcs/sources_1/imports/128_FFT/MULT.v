module MULT(data_in_re,data_in_im,cnt,data_out_re,data_out_im);
	...
	always@(*) begin
    	...
    	else if (N==32) begin // 1ë§Œí¼ shift ?•„?š”
            case(cnt[4:0])
                5'd1 : begin
                       temp_re <= W_re0;
                       temp_im <= W_im0;
...
assign data_out_re = {buf_re[BW+12],buf_re[BW+8:10]};  // 16 bit Okay!
